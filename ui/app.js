...
(() => {
  ...
  function kpiCard(label, value, support, iconName, tone = "primary") {
    return `<article class="kpi-card"><div class="kpi-top"><span class="kpi-label">${escapeHtml(label)}</span><span class="kpi-icon ${tone}">${icon(iconName)}</span></div><div class="kpi-value">${escapeHtml(value)}</div><div class="kpi-support">${support}</div></article>`;
  }
